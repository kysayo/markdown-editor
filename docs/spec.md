...
- **設定ダイアログ** — `表示(V)` → `設定(P)...` で各種表示パラメーターをリアルタイム調整
  - 見出しの余白：0.3〜2.0 em（デフォルト 0.9 em）
  - 箇条書きの余白：0.0〜1.5 em（デフォルト 0.75 em）
  - 表の行間：0.0〜1.0 em（デフォルト 0.3 em）— セルの縦 padding を制御
  - 表のインデント：0.0〜5.0 em（デフォルト 0 em）— テーブルの左マージンを制御
  - スライダーと数値入力の両方で調整可能
  - 設定は `localStorage`（`md-editor-settings`キー）に永続化され、起動時に復元される
  - CSS変数 `--heading-spacing` / `--list-spacing` / `--table-cell-padding-v` / `--table-indent` を `useEffect` でリアルタイム更新する
- **文字サイズズーム** — エディター本文のフォントサイズをリアルタイムで変更（10〜32px）
  - `Ctrl + ホイール上` で拡大、`Ctrl + ホイール下` で縮小
  - `Ctrl + =` / `Ctrl + +` で拡大、`Ctrl + -` で縮小
  - `Ctrl + 0` でデフォルト（16px）にリセット
  - WYSIWYGモード・ソースモード両方に適用
  - ステータスバー右端に現在のフォントサイズ（px）を表示
- **ドラッグ&ドロップでファイルを開く** — `.md` ファイルをウィンドウにドロップすると即座に開く（Tauri `onDragDropEvent` を使用）
- **ファイル関連付け起動** — エクスプローラーで `.md` ファイルをダブルクリックするとそのファイルを開いて起動する（Rust 側で CLI 引数を受け取り内容ごと返す方式。`readTextFile` の scope 制限を回避）
...
| Ctrl + B | 太字 | 実装済み |
| Ctrl + D | 打ち消し線 | 実装済み |
| Ctrl + U | 下線（ソースモードでは `<u>text</u>`） | 実装済み |
| Ctrl + Shift + E | 文字修飾を消す | 実装済み |
| Ctrl + Shift + K | コードブロック | 実装済み |
| Ctrl + Shift + Q | 引用ブロック | 実装済み |
| Ctrl + Shift + T | テーブル挿入ダイアログ | 実装済み |
| Ctrl + Shift + V | プレーンテキスト貼り付け | 実装済み |
| Ctrl + S | 上書き保存（未保存なら名前を付けて保存） | 実装済み |
| Ctrl + Shift + S | 名前を付けて保存 | 実装済み |
| Ctrl + O | ファイルを開くダイアログ（空タブなら上書き、そうでなければ新タブ） | 実装済み |
| Ctrl + T | 新しい空タブを作成（同一ウィンドウ） | 実装済み |
| Ctrl + W | アクティブタブを閉じる（最後の1タブならアプリ終了） | 実装済み |
| Ctrl + N | 新しいウィンドウで空ファイルを開く | 実装済み |
| Ctrl + Shift + N | 新しいウィンドウで空ファイルを開く（Ctrl+N と同じ） | 実装済み |
| Alt + ← / → | 前/次のタブに切り替え | 実装済み |
| Ctrl + ホイール | 文字サイズ拡大/縮小 | 実装済み |
| Ctrl + = / + | 文字サイズ拡大 | 実装済み |
| Ctrl + - | 文字サイズ縮小 | 実装済み |
| Ctrl + 0 | 文字サイズをデフォルト（16px）にリセット | 実装済み |

- Ctrl + U の注意: ソースモードでは `<u>text</u>` として保存される。ファイル再オープン時に下線が失われる既知の制限あり（将来対応予定）

### タブ機能（実装済み）

| 操作 | 動作 |
|---|---|
| Ctrl + T | 新しい空タブを作成（同一ウィンドウ） |
| Ctrl + W | アクティブタブを閉じる。最後の1タブならアプリ終了 |
| Ctrl + N / Ctrl + Shift + N | 新しいウィンドウで空ファイルを開く |
| Ctrl + O | アクティブタブが空なら上書き、そうでなければ新タブで開く |
| D&D | アクティブタブが空なら上書き、そうでなければ新タブで開く |
| Alt + ← / Alt + → | 前/次のタブに切り替え |

- タブが1つのときはタブバーを非表示にする（現在の見た目を維持）
- 「空タブ」の定義：`filePath` が未設定、かつエディター内容が空白のみ
- OSのエクスプローラーからファイルを選択して起動するケースは実装済み（ダブルクリック起動対応）
...
│   ├── main.tsx
│   └── store/
│       ├── fileStore.ts        # Zustand: Tab配列 / activeTabId
│       └── settingsStore.ts    # Zustand: headingSpacing / listSpacing / tableCellPaddingV / tableIndent（localStorage永続化）
├── docs/
│   └── spec.md                 # 本ファイル
...
| 表示設定（余白・表・インデント調整）         | ✅ 実装済み         | —         |
| 文字サイズズーム                   | ✅ 実装済み         | —         |
| ドラッグ&ドロップでファイルを開く         | ✅ 実装済み         | —         |
| ファイル関連付け起動（ダブルクリック）       | ✅ 実装済み         | —         |
